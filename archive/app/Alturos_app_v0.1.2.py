...
# Split the layout 
simulation, all_data = st.columns(2)

if simulation.button('Simulation'):
    ...
    try:
        ...
        # animation = data_animation.the_thing(data, model_path)

        # # Show animation 
        # st.pyplot(animation)

        # Assuming data is loaded and stored in `data`
        data_animation_instance = DataAnimation(data)
        animation = data_animation_instance.the_thing(model_path)
        
        # Convert animation to HTML/JS and display
...
# # Update UI dynamically based on button selection
# elif simulation.button_clicked:
#     # Clear the other column
#     all_data.empty()     

# elif all_data.button_clicked:
#     # Clear the other column
#     simulation.empty()  
